...
if dataset == 'synthetic':
    ...
    df = pd.read_csv('./data/mammographic_masses.data', header=None)
    ...
    # check the distribution of the data for exam_idx region 
    y_train[x_train[:,0] <= np.quantile(x_train[:,0], 0.3)].mean()
    x = torch.tensor(x_train, dtype=torch.float32)
    y = torch.tensor(y_train, dtype=torch.long).reshape(-1)
    x_test = torch.tensor(x_test, dtype=torch.float32)
    y_test = torch.tensor(y_test, dtype=torch.long).reshape(-1)
    ...
    df.columns = ['id', 'clump_thickness', 'uniformity_cell_size', 'uniformity_cell_shape', 'marginal_adhesion', 'single_epithelial_cell_size', 'bare_nuclei', 'bland_chromatin', 'normal_nucleoli', 'mitoses', 'class']
    del df['id']
    df = df.replace('?', np.nan)
    df = df.fillna(df.mode().iloc[0])
    x = df.iloc[:,:-1].values
    ...
    columns_used = ['Age','Number of sexual partners','First sexual intercourse','Num of pregnancies','Smokes','Smokes (years)','Smokes (packs/year)','Hormonal Contraceptives','Hormonal Contraceptives (years)','IUD','Biopsy']
    df = df[columns_used]
    df = df.replace('?', np.nan)
    df = df.fillna(-1)
    x = df.iloc[:,:-1].values
    ...
    attrib = pd.read_csv('./data/communities.names', delim_whitespace=True)
    df = pd.read_csv('./data/communities.data', names = attrib['attributes'])
    used_columns = ['population', 'householdsize', 'racepctblack', 'racePctWhite', \
        'medIncome', 'NumImmig','PolicCars','ViolentCrimesPerPop']
    df = df[used_columns]
    df = df.replace('?', np.nan)
    df = df.fillna(df.mode().iloc[0])

    x = df.iloc[:,:-1].values
    y = (df['ViolentCrimesPerPop'] > np.quantile(df['ViolentCrimesPerPop'], 0.5)).astype(int).values
...
class Human:
    ...
    def hbm_init(self, x, y=None):
        ...
        if dataset == 'synthetic':
            ...
            # use two segments of BI-RADS and age 
            if x[0] <= np.quantile(x_train[:,0], 0.7) and x[1] <= np.quantile(x_train[:,1], 0.7):
                # low BI-RADS - definitely benign and low age 
                prob = 0.1
            elif x[0] <= np.quantile(x_train[:,0], 0.7) and x[1] > np.quantile(x_train[:,1], 0.7):
                # low BI-RADS - definitely benign and high age
                prob = 0.1
            elif x[0] > np.quantile(x_train[:,0], 0.7) and x[1] <= np.quantile(x_train[:,1], 0.7):
                # high BI-RADS - likely malignant and low age
                prob = 0.9 
            else:
                # high BI-RADS - likely malignant and high age
                prob = 0.3
        elif dataset == 'bcwisc':
            # use two segments of clump_thickness and uniformity_cell_size
            if x[0] <= np.quantile(x_train[:,0], 0.3) and x[1] <= np.quantile(x_train[:,1], 0.7):
                # low clump_thickness and low uniformity_cell_size
                prob = 0.9
            elif x[0] <= np.quantile(x_train[:,0], 0.3) and x[1] > np.quantile(x_train[:,1], 0.7):
                # low clump_thickness and high uniformity_cell_size
                prob = 0.3
            elif x[0] > np.quantile(x_train[:,0], 0.3) and x[1] <= np.quantile(x_train[:,1], 0.7):
                # high clump_thickness and low uniformity_cell_size
                prob = 0.
            else:
                # high clump_thickness and high uniformity_cell_size
                prob = 0.0
        elif dataset == 'cervicalcancer':
            # cone biopsy may increase the risk for infertility and miscarriage. https://www.hopkinsmedicine.org/health/treatment-tests-and-therapies/cervical-biopsy
            # Lee, N. H. "The effect of cone biopsy on subsequent pregnancy outcome." Gynecologic Oncology 6.1 (1978): 1-6.
            # so the doctor is less likely to perform biopsy on patients who are young and have low number of pregnancies - may not have kids 
            if x[0] <= np.quantile(x_train[:,0], 0.7) and x[3] <= np.quantile(x_train[:,3], 0.7):
                prob = 0.
            elif x[0] <= np.quantile(x_train[:,0], 0.7) and x[3] > np.quantile(x_train[:,3], 0.7):
                prob = 0.3
            elif x[0] > np.quantile(x_train[:,0], 0.7) and x[3] <= np.quantile(x_train[:,3], 0.7):
                prob = 0.3
            else:
            ...
            # used_columns = ['population', 'householdsize', 'racepctblack', 'racePctWhite', 'medIncome', 'NumImmig','PolicCars','ViolentCrimesPerPop']
            # high white population and high income neighborhood 
            if x[0] > np.quantile(x_train[:,0], 0.3) and x[4] > np.quantile(x_train[:,4], 0.5):
                prob = 0 
            elif x[0] > np.quantile(x_train[:,0], 0.3) and x[4] <= np.quantile(x_train[:,4], 0.5):
                # high white population and low income neighborhood
                prob = 0
            elif x[0] <= np.quantile(x_train[:,0], 0.3) and x[4] > np.quantile(x_train[:,4], 0.5):
                # low white population and high income neighborhood
                prob = 0.3
            else:
                # low white population and low income neighborhood
                prob = 0.9
        elif dataset == 'mushroom':
            if x[1] < 0 and x[4] > 0:
                prob = 0.0
...
if dataset == 'gmc' or dataset == 'breastcancer' or dataset == 'claims' or dataset == 'bcwisc' or dataset == 'cervicalcancer' or dataset == 'law' or dataset == 'credit' or dataset == 'communities':
    hidden = 8 
else:
    hidden = 32
model = nn.Sequential(
...
for step in range(args.steps):
    ...
    if dataset == 'synthetic_moon':
        ...
        exam_idx = (x_test[:,0] > np.quantile(x_train[:,0], 0.3)) 
        exam_idx = ~exam_idx
    elif dataset == 'cervicalcancer':
        exam_idx = (x_test[:,0] <= np.quantile(x_train[:,0], 0.7)) & (x_test[:,3] <= np.quantile(x_train[:,3], 0.7))
        exam_idx = ~exam_idx
    elif dataset == 'credit':
        ...
        #exam_idx = (x_test[:,2] > np.quantile(x_train[:,2], 0.3)) & (x_test[:,4] > np.quantile(x_train[:,4], 0.3))
        exam_idx = (x_test[:,0] > np.quantile(x_train[:,0], 0.3))
        exam_idx = ~exam_idx
    elif dataset == 'mushroom':
        exam_idx = (x_test[:,1] < 0)
