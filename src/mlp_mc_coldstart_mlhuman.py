...
# randomly pick 10 samples to create a human model
# logisitic regression model
seed_everything(seed)
from sklearn.linear_model import LogisticRegression
from sklearn.ensemble import RandomForestClassifier
human_mlmodel = LogisticRegression(random_state=seed, penalty='l1', solver='liblinear')
#human_mlmodel = RandomForestClassifier(random_state=seed)

if dataset == 'mushroom':
    init_human_numindex = 3
else:
    init_human_numindex = 3

#human_random_index = np.random.choice(np.arange(len(x_train)), init_human_numindex, replace=False)
# random select 3 positive and 3 negative samples
human_random_positive_index = np.random.choice(np.where(y_train == 1)[0], init_human_numindex, replace=False)
human_random_negative_index = np.random.choice(np.where(y_train == 0)[0], init_human_numindex, replace=False)
human_random_index = np.concatenate([human_random_positive_index, human_random_negative_index], axis=0)
human_x = x_train[human_random_index]
human_y = y_train[human_random_index]
human_mlmodel.fit(human_x, human_y)
# accuracy of the human model
human_prediction = human_mlmodel.predict(x_test)
human_accuracy = np.mean(human_prediction == y_test.numpy())
print(f'human model accuracy: {human_accuracy}')

if dataset == 'mushroom':
    half_thres = np.quantile(human_mlmodel.predict_proba(x_train)[:,1], 0.5)
    acc_thres = np.quantile(human_mlmodel.predict_proba(x_train)[:,1], 0.7)
elif dataset == 'gmc':
    half_thres = np.quantile(human_mlmodel.predict_proba(x_train)[:,1], 0.7)
    acc_thres = np.quantile(human_mlmodel.predict_proba(x_train)[:,1], 0.9)
elif dataset == 'adult':
    half_thres = np.quantile(human_mlmodel.predict_proba(x_train)[:,1], 0.5)
    acc_thres = np.quantile(human_mlmodel.predict_proba(x_train)[:,1], 0.7)
else:
...
class Human:
    ...
    def hbm_init(self, x, y=None):
        ...
        if dataset == 'synthetic':
            ...
            # most requests will be rejected 
            if prob > acc_thres:
                prob = 0.9
            elif prob > half_thres and prob <= acc_thres:
                prob = 0.3
            else:
                prob = 0.0
        elif dataset == 'mushroom':
            ...
            # use human model to reject the samples
            #prob = human_mlmodel.predict_proba(x.reshape(1,-1))[0][1]
            prob = self.model.predict_proba(x.reshape(1,-1))[0][1]
            # most requests will be rejected 
            thres = 0.7 
            if prob > thres:
        ...
        elif dataset == 'cirvicalcancer':
            # use human model to reject the samples
            #prob = human_mlmodel.predict_proba(x.reshape(1,-1))[0][1]
            # more likely to perform biopsy if likelihood of cancer is high
            prob = self.model.predict_proba(x.reshape(1,-1))[0][1]
            prob = np.clip(prob, 0.1, 0.9)
        elif dataset == 'law':
...
if dataset == 'gmc' or dataset == 'breastcancer' or dataset == 'claims' or dataset == 'law' or dataset == 'bcwisc' or dataset == 'cirvicalcancer':
    hidden = 8 
else:
    hidden = 32
model = nn.Sequential(
...
for step in range(args.steps):
    ...
    if dataset in ['synthetic', 'synthetic_moon', 'synthetic_moon2', 'synthetic_rect', 'synthetic_circle']:
        ...
        plt.legend(loc = 'upper center', ncol = 3, bbox_to_anchor=(0.5, 1.1), fancybox=True, shadow=True, fontsize='x-small')
    ...
    if dataset == 'synthetic_moon':
        ...
        label_prob = human_mlmodel.predict_proba(x_test)[:,1]
        exam_idx = (label_prob > half_thres)
    elif dataset == 'breastcancer':
        exam_idx = (x_test[:,0] > -np.inf) 
    elif dataset == 'bcwisc':
        exam_idx = (x_test[:,0] > -np.inf) 
    elif dataset == 'cervicalcancer':
        exam_idx = (x_test[:,0]>-np.inf)
    ...
    results.to_csv(f'./log_mlhuman/coldstart_{dataset}_{method}_{seed}_{args.beta}_results.csv', index=False)
