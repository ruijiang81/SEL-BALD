...
class Naive_BALD(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
...
class Joint_Naive_BALD(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
    ...
    def get_ranks_human(self, predictions, alpha = 0.5):
        """
        Rank the predictions according to their uncertainties.
        ...
        """
        predictions, hbm_predictions = predictions
        if isinstance(predictions, types.GeneratorType):
            scores = self.get_uncertainties_generator(predictions)
            hbm_scores = self.get_uncertainties_generator(hbm_predictions)
        else:
            scores = self.get_uncertainties(predictions)
            hbm_scores = self.get_uncertainties(hbm_predictions)

        # logit to prob
        hbm_predictions = to_prob(hbm_predictions)
        avg_hbm_predictions = np.mean(hbm_predictions, axis=-1)
        #avg_hbm_predictions = np.quantile(hbm_predictions, 0.75, axis=-1)

        #scores = (1-alpha) * scores * avg_hbm_predictions[:,1] + alpha * hbm_scores
        scores = (1-alpha) * scores + alpha * hbm_scores
        scores = scores * avg_hbm_predictions[:,1]
        return self.reorder_indices(scores), scores


class Joint_BALD(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
    ...
    def get_ranks_human(self, predictions, alpha = 0.5):
        ...
        y_score = scores
        avg_human_pred = avg_hbm_predictions[:,1]
        h_score = hbm_scores
        scores = (1-alpha) * scores * avg_hbm_predictions[:,1] + alpha * hbm_scores

        # plus a gumbel noise (0, gamma^-1)
        #gamma = 1 
        #scores = scores + np.random.gumbel(0, 1/gamma, scores.shape)
        return self.reorder_indices(scores), scores

class Joint_BALD_UCB(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
    ...
    def __init__(self, beta = 0.75, shuffle_prop=DEPRECATED, reduction="none"):
        super().__init__(shuffle_prop=shuffle_prop, reverse=True, reduction=reduction)
        self.beta = beta
    ...
    def get_ranks_human(self, predictions, alpha = 0.5):
        ...
        # logit to prob
        hbm_predictions = to_prob(hbm_predictions)
        avg_hbm_predictions = np.quantile(hbm_predictions, self.beta, axis=-1)
...
class Joint_BALD_TS(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
    ...
    def get_ranks_human(self, predictions, alpha = 0.5):
        ...
        hbm_predictions = to_prob(hbm_predictions)
        #avg_hbm_predictions = np.quantile(hbm_predictions, self.beta, axis=-1)
        hbm_predictions = hbm_predictions[:,1,:]
        # random sample one from each column
        rand_index = np.random.choice(hbm_predictions.shape[1], hbm_predictions.shape[0])
        hbm_predictions_sampled = hbm_predictions[range(hbm_predictions.shape[0]), rand_index]

        alpha = 0.5 
        #scores = (1-alpha) * scores * avg_hbm_predictions[:,1] + alpha * hbm_scores
        scores = (1-alpha) * scores * hbm_predictions_sampled + alpha * hbm_scores

        #gamma = 1 
        #scores = scores + np.random.gumbel(0, 1/gamma, scores.shape)             
        return self.reorder_indices(scores), scores

class e_BALD(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
...
class e_BALD_UCB(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
...
class e_BALD_true(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
    ...
    def __init__(self, shuffle_prop=DEPRECATED, reduction="none", dataset=None, x_train = None):
        super().__init__(shuffle_prop=shuffle_prop, reverse=True, reduction=reduction)
...
class e_Random(AbstractHeuristic):
    """
    Sort by the highest acquisition function value.
    ...
    def get_ranks_human(self, predictions, alpha = 0.5):
        ...
        # random scores 
        scores = np.random.rand(scores.shape[0])
        # logit to prob
        hbm_predictions = to_prob(hbm_predictions)
        avg_hbm_predictions = np.mean(hbm_predictions, axis=-1)
...
class e_Entropy(AbstractHeuristic):
    """
    ...
    def compute_score(self, predictions):
        ...
        Returns:
            Array of scores.
        """
        entropy = scipy.stats.entropy(np.swapaxes(predictions, 0, 1))
        return entropy 
    ...
    def get_ranks_human(self, predictions, alpha = 0.5):
        ...
        if isinstance(predictions, types.GeneratorType):
            scores = self.compute_score(predictions)
            hbm_scores = self.compute_score(hbm_predictions)
        else:
            scores = self.compute_score(predictions)
            hbm_scores = self.compute_score(hbm_predictions)
        ...
        hbm_predictions = to_prob(hbm_predictions)
        #avg_hbm_predictions = np.mean(hbm_predictions, axis=-1)
        avg_hbm_predictions = np.quantile(hbm_predictions, 0.9, axis=-1)

        scores = scores * avg_hbm_predictions[:,1] + hbm_scores
        return self.reorder_indices(scores), scores
...
class Random(Precomputed):
    ...
    def compute_score(self, predictions):
        ...
        this_seed = np.random.randint(0, 100000)
        rs = np.random.RandomState(this_seed)
        return rs.rand(predictions.shape[0])
