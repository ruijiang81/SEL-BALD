...
class ActiveLearningDataset(SplittedDataset):
    ...
    def label_randomly_human(self, n=1, human=None) -> None:
        indices = self.random_state.choice(self.n_unlabelled, n, replace=False).tolist()
        labeled_ins = []
        for i in indices:
            x, y = self.pool[i]
            prob, label = human.hbm(x.numpy())
            if label == 1:
                labeled_ins.append(i)
        oracle_indices = self._pool_to_oracle_index(indices)
        if len(labeled_ins) > 0:
            self.label(labeled_ins)
        return oracle_indices

    def label_randomly_human_init(self, n=1, human=None) -> None:
        indices = self.random_state.choice(self.n_unlabelled, n, replace=False).tolist()
        labeled_ins = []
        for i in indices:
            x, y = self.pool[i]
            prob, label = human.hbm_init(x.numpy(), y)
            if label == 1:
                labeled_ins.append(i)
        oracle_indices = self._pool_to_oracle_index(indices)
        if len(labeled_ins) > 0:
